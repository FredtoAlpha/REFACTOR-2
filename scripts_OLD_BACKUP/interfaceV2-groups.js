...
function initGroupsModule() {
  console.log('🚀 Initialisation du module de groupes (version consolidée)...');
...
function createEnhancedGroupsMenu() {
  ...
  if (!nav) {
    ...
    console.log('Debug: Vérifions si app-header existe:', document.querySelector('.app-header'));
    return;
  }

  console.log('✅ Nav trouvée, création du bouton...');

  // Remplacer le menu existant
  const existingWrapper = document.querySelector('#btnGroups')?.parentElement;
  if (existingWrapper) {
    console.log('⚠️ Menu GROUPES existant trouvé, suppression...');
  ...
  // Insérer dans la navigation (avant le menu Paramètres)
  const settingsMenu = document.getElementById('settingsMenuWrapper');
  if (settingsMenu) {
    nav.insertBefore(groupsWrapper, settingsMenu);
    console.log('✅ Bouton GROUPES inséré AVANT le menu Paramètres');
  } else {
    nav.insertBefore(groupsWrapper, nav.firstChild);
    console.log('✅ Bouton GROUPES inséré au début de la nav');
  }

  // Mettre à jour le compteur
  updateGroupsButton();
  console.log('🎉 Menu GROUPES créé avec succès !');
}
...
// 6. Remplacer l'ancien menu par le nouveau au chargement - SUPPRIMÉ (REDONDANT)
// createEnhancedGroupsMenu() est déjà appelée par initGroupsModule() ligne 10768
// qui possède son propre listener DOMContentLoaded ligne 10774
// Cet appel redondant causait une double initialisation du menu

// ========== BOUTON SAUVEGARDER - Désormais dans le header principal ==========
// Le bouton btnSaveMain est géré directement dans la section d'initialisation principale
// Plus besoin de correction, le bouton est proprement configuré dès le départ
    ...
    if (!window.saveProgressManager) {
      ...
      try {
        ...
        if (result?.success) {
          ...
          toast('Erreur lors de la création des onglets FIN', 'error');
        }
      } catch (error) {
