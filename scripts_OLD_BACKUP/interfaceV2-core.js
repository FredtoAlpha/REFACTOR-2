// Scripts principaux extraits de InterfaceV2.html pour améliorer la lisibilité.
// =======================================================
...
class SaveProgressManager {
  ...
  reset() {
    ...
    const header = this.progressBar?.querySelector('.save-progress-header span');
    if (header) {
      header.innerHTML = '<i class="fas fa-save mr-2"></i>Création des onglets FIN...';
...
async function saveWithProgressINT() {
  ...
  try {
    ...
    // Étape 3 : Création des onglets FIN (75%)
    ...
    console.error('Erreur création onglets FIN:', error);
...
function gsRun(fnName, ...args) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // ✅ Google Apps Script non disponible
      reject(new Error('Google Apps Script non disponible. Assurez-vous que l\'application est déployée.'));
...
// VERSION 4.0 - Octobre 2025 - InterfaceV2
// Amélioration complète : gestion prénoms composés, traits d'union, longueur max
console.log('🔄 VERSION 4.0 - Fonction simplifierNomComplet améliorée chargée dans InterfaceV2');

function simplifierNomComplet(nom, prenom) {
  // VERSION 4.0 - Logique complète de simplification
  ...
  // 1. Simplifier le nom de famille (garder uniquement le premier mot)
  const nomSimple = nom.split(/[\s-]+/)[0];

  // 2. Simplifier le prénom si nécessaire
  let prenomSimple = prenom;

  // Si le prénom contient des espaces ou traits d'union
  if (/[\s-]/.test(prenom)) {
    const parts = prenom.split(/[\s-]+/);
    // Garder le premier prénom, abréger les suivants
    prenomSimple = parts[0];
    if (parts.length > 1 && parts[0].length < 8) {
      // Ajouter l'initiale du second prénom si le premier est court
      prenomSimple += `-${parts[1].charAt(0)}.`;
    }
  }

  // Si le prénom est très long (> 12 caractères), le tronquer
  ...
  // 4. Vérifier la longueur totale (max 25 caractères pour l'affichage)
  const MAX_LENGTH = 25;
  if (resultat.length > MAX_LENGTH) {
    // Réduire encore plus le prénom si nécessaire
    const nomLength = nomSimple.length;
    const availableForPrenom = MAX_LENGTH - nomLength - 1; // -1 pour l'espace
  ...
  // Debug pour vérifier que la fonction fonctionne
  if (nom !== nomSimple || prenom !== prenomSimple) {
    console.log(`✂️ InterfaceV2 v4.0 - Nom simplifié: "${nom} ${prenom}" → "${resultat}" (${resultat.length} car.)`);
  }

  return resultat;
}

// Marquer la version pour éviter les conflits
...
if (typeof window !== 'undefined') {
  ...
  console.log('✅ simplifierNomComplet VERSION 4.0 exposée globalement depuis InterfaceV2');
}
...
function createClassColumn(classe, eleves) {
  ...
  // Restaurer l'état visuel du tri si existant
...
function showModeBadge(mode) {
  ...
  if (badge) {
    // Déterminer le label et la couleur selon le mode
    let label = 'MODE ' + mode;
    let bgColor = '#3b82f6'; // Bleu par défaut

    switch(mode) {
      case 'TEST':
        label = '📚 TEST (Classes test)';
        bgColor = '#3b82f6'; // Bleu
        break;
      case 'PREVIOUS':
        label = '📋 PREVIOUS (Année passée)';
        bgColor = '#8b5cf6'; // Violet
        break;
      case 'FINAL':
        label = '✅ FINAL (Classes finales)';
        bgColor = '#10b981'; // Vert
        break;
      // Anciens modes pour rétrocompatibilité
      case 'CACHE':
        label = 'MODE CACHE';
...
window.addEventListener('DOMContentLoaded', (event) => {
  ...
  // Vérifier si une sauvegarde CACHE existe
  checkCache();
  ...
  function applyPreferences() {
    ...
    // Gérer l'affichage des prénoms (nom seul si décochée)
    ...
    allNames.forEach(nameEl => {
      ...
      if (lisibiliteState.showPrenoms) {
        // Afficher nom + prénom
        nameEl.textContent = `${eleve.nom} ${eleve.prenom}`;
      } else {
  ...
  function toggleSwapMode() {
    ...
    if (STATE.swapMode) {
      ...
      toast('Mode normal rétabli', 'info');
    }
  }
...
function openStartupModal(opts = {}) {
  ...
  // Si le tableau de répartition est déjà affiché, ne rien faire
  const board = document.getElementById('board');
  ...
  checkCache(); // Vérifier et afficher la dernière sauvegarde automatique
}
...
window.updateCharts = updateCharts;
  ...
  function setupEventListeners() {
    ...
    async function lancerSauvegardeAvecProgression(saveFunctionName, buttonElement, options = {}) {
      const { title = 'SAUVEGARDE EN COURS...', successMessage = 'Sauvegarde réussie', progressive = true } = options;

      if (!window.saveProgressManager) {
        console.error('SaveProgressManager non trouvé !');
      ...
      try {
        ...
        // ✅ SAUVEGARDE PROGRESSIVE CLASSE PAR CLASSE (évite les timeouts)
        if (progressive && saveFunctionName === 'saveElevesCache') {
          ...
          console.log(`📦 Sauvegarde progressive de ${totalClasses} classe(s)...`);

          for (let i = 0; i < totalClasses; i++) {
            const classe = classes[i];
            const classMap = { [classe]: disposition[classe] };

            // Mettre à jour la progression (20% à 90%)
            const progress = 20 + ((i / totalClasses) * 70);
            window.saveProgressManager.updateProgress(progress, 'step3');

            // Mise à jour du titre avec le nom de la classe en cours
            if(header) header.innerHTML = `<i class="fas fa-save mr-2"></i>${title} - ${classe} (${i+1}/${totalClasses})`;

            try {
              // ✅ Sauvegarder UNE classe à la fois
              const result = await gsRun(saveFunctionName, classMap, STATE.currentMode);

              if (!result || !result.success) {
                errors.push(`❌ ${classe}: ${result?.message || 'Erreur inconnue'}`);
                console.error(`❌ Échec sauvegarde ${classe}:`, result);
              } else {
                successCount++;
                console.log(`✅ ${classe} sauvegardée (${successCount}/${totalClasses})`);
              }
            } catch (err) {
              errors.push(`❌ ${classe}: ${err.message}`);
              console.error(`❌ Exception sauvegarde ${classe}:`, err);
            }

            // Petit délai pour éviter les timeouts consécutifs
          ...
          if (successCount === totalClasses) {
            window.saveProgressManager.complete(true);
            toast(`✅ ${successCount} classe(s) sauvegardée(s) avec succès !`, 'success');
          } else if (successCount > 0) {
            window.saveProgressManager.complete(true);
            toast(`⚠️ ${successCount}/${totalClasses} classe(s) sauvegardée(s). Erreurs: ${errors.join(', ')}`, 'warning');
            console.warn('Erreurs de sauvegarde:', errors);
          } else {
            throw new Error('Aucune classe sauvegardée: ' + errors.join(', '));
          }

        } else {
          // ✅ SAUVEGARDE CLASSIQUE (pour finalizeClasses et autres)
          window.saveProgressManager.updateProgress(50, 'step2');
          await new Promise(resolve => setTimeout(resolve, 300));
    ...
    // Bouton FINALISER - Création des classes définitives
    if (btnFinalize) btnFinalize.addEventListener('click', async (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (!confirm('✅ Créer les classes finales définitives ? Cette action est irréversible.')) return;
    ...
    if (showGenderBadges) {
      ...
      showGenderBadges.addEventListener('change', (e) => {
        ...
        if (isChecked) {
          document.body.classList.add('gender-badges');
          toast('Badges sexe activés', 'success');
        } else {
          document.body.classList.remove('gender-badges');
          toast('Badges sexe désactivés', 'info');
        }
      });
    }
    ...
    if (whiteBackground) {
      ...
      whiteBackground.addEventListener('change', (e) => {
        ...
        if (isChecked) {
          document.body.classList.add('white-bg');
          toast('Fond blanc activé', 'success');
        } else {
          document.body.classList.remove('white-bg');
          toast('Fond blanc désactivé', 'info');
        }
      });
    }
...
async function initRepartitionApp() {
  ...
  // Arrêter les anciennes sauvegardes automatiques pour être propre
  ...
  // Démarrer la sauvegarde automatique dans le navigateur (uniquement si chargement réussi)
...
window.gsRun = gsRun;
  ...
  (function() {
    ...
    if (settingsDropdown) {
      ...
      // Boutons Favoris, Optimiser, Contraintes, Filtres supprimés (non fonctionnels)
...
// 1. Fonction unifiée openGroupsInterface - utilise le nouveau GroupsModuleComplete (modal intégrée)
function openGroupsInterface(tab = 'creator') {
  console.log('🚀 Ouverture de l\'interface des groupes via GroupsModuleComplete...');

  // Vérifier si GroupsModuleComplete est disponible (groupsModuleComplete.html)
  if (typeof window !== 'undefined' && window.GroupsModuleComplete && typeof window.GroupsModuleComplete.open === 'function') {
    try {
      ...
      console.log('✅ GroupsModuleComplete ouvert avec succès');
      return;
    } catch (error) {
      console.error('❌ Erreur lors de l\'ouverture de GroupsModuleComplete:', error);
    }
  }

  // FALLBACK : Si GroupsModuleComplete n'est pas disponible, utiliser l'ancienne interface popup
  console.warn('⚠️ GroupsModuleComplete non disponible, fallback vers interface popup...');
  google.script.run
        ...
        const checkInterval = setInterval(() => {
          try {
            if (groupsWindow.GroupsUI && typeof groupsWindow.GroupsUI.switchTab === 'function') {
              ...
              console.log('✅ Interface des groupes popup ouverte avec succès');
            }
          } catch (uiError) {
            clearInterval(checkInterval);
          }
...
// 5. Amélioration du menu dropdown des groupes - SUPPRIMÉ
// Cette fonction est obsolète, remplacée par createEnhancedGroupsMenu() ligne 10828
// L'initialisation se fait via initGroupsModule() qui appelle createEnhancedGroupsMenu()

// 6. Initialisation au chargement - SUPPRIMÉ (conflit avec initGroupsModule ligne 10771)
